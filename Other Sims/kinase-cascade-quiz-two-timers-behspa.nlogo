; quiz problem -- protein kinase cascade
; proteinA activates protein B (red --> light red)
; proteinB when activated, activates protein C (blue --> light blue)
; proteinC returns to blue after timer runs out
; proteinB is never inactivated, so eventually all will be light red

breed [proteinAs proteinA]
breed [proteinBs proteinB]
breed [proteinCs proteinC]
proteinBs-own [Bcolor-timer]
proteinCs-own [color-timer]
...
to make-proteins
  create-proteinAs num-ProteinA [
    setxy random-xcor random-ycor
    set size 4
    set shape "circle"
    set color white
  ]
  create-proteinBs num-ProteinB [
    setxy random-xcor random-ycor
    set size 3
    set shape "circle"
    set color red
  ]
  create-proteinCs num-ProteinC [
    setxy random-xcor random-ycor
    set size 2
    set shape "circle"
    set color blue
...
to proteinA-acts
  ask proteinAs [
    move
    if any? proteinBs-here
      [let my-B one-of proteinBs-here
        ask my-B [
          if Bcolor-timer = -1
            [set color magenta
              set Bcolor-timer  active-time-ProteinB
            ]
          ]
       ]
    ]
...
to proteinB-acts
  ask proteinBs [
    move
    if Bcolor-timer >= 0
      [set Bcolor-timer Bcolor-timer - 1
        if Bcolor-timer = -1 [set color red]
      ]
    if color = magenta
      [if any? proteinCs-here
        [let my-C one-of proteinCs-here
         ask my-C [
           if color-timer = -1
            [set color cyan
              set color-timer active-time-ProteinC
            ]
          ]
        ]
      ]
...
"" ""
PENS
"default" 1.0 0 -7858858 true "" "plot count proteinBs with [color = magenta]"
"pen-1" 1.0 0 -11221820 true "" "plot count proteinCs with [color = cyan]"
...
<experiments>
  <experiment name="experiment" repetitions="5" runMetricsEveryStep="false">
    ...
    <metric>count ProteinBs with [color = magenta]</metric>
    <metric>count ProteinCs with [color = cyan]</metric>
    <enumeratedValueSet variable="active-time-proteinC">
      <value value="500"/>
      <value value="1000"/>
      <value value="1500"/>
    </enumeratedValueSet>
    <enumeratedValueSet variable="active-time-ProteinB">
      <value value="500"/>
      <value value="1000"/>
      <value value="1500"/>
    </enumeratedValueSet>
    <enumeratedValueSet variable="num-ProteinA">
      <value value="40"/>
    </enumeratedValueSet>
    <enumeratedValueSet variable="num-ProteinB">
      <value value="50"/>
    </enumeratedValueSet>
    <enumeratedValueSet variable="num-ProteinC">
      <value value="50"/>
    </enumeratedValueSet>
  </experiment>
