;; Lassa Fever Spread in Nigeria

breed [people person] people-own [health hunger infected?]
breed [rats rat] rats-own [infected? hunger age]
patches-own [infected countdown]
...
to set-citybackground
  ifelse warm-weather?
  [ask patches with [pycor >= 40] [set pcolor 8.9]
    ask patches with [pycor < 40 ] [set pcolor 56]] ;if true

  [ask patches with [pycor >= 20] [set pcolor 8.9]
  ask patches with [pycor < 20] [set pcolor 56]] ;if false
end


to set-farms
  ifelse warm-weather?
  [ask patches with [pxcor > 10 and pxcor < 40 and pycor > 10 and pycor < 30] [set pcolor one-of [53 36]]
    ask patches with [pxcor > 55 and pxcor < 85 and pycor > 10 and pycor < 30] [set pcolor one-of [53 36]]
    ask patches [if pcolor = 53 [set countdown 2]]
    grass-regrows]

  [ask patches with [pxcor > 40 and pxcor < 60 and pycor > 5 and pycor < 15] [set pcolor one-of [53 36]]
    ask patches [if pcolor = 53 [set countdown 10]]
    grass-regrows]
...
to set-village1
  ask patches with [pxcor > 10 and pxcor < 40 and pycor > 60 and pycor < 90] [set pcolor pink - 1 set infected false]
end

to set-village2
  ask patches with [pxcor > 60 and pxcor < 90 and pycor > 60 and pycor < 90] [set pcolor blue + 1 set infected false]
end


;to set-hospitals
;  ifelse dense?
;    [ask n-of 4 patches with [pycor > 50] [ask n-of 1 patches in-radius 1 [set pcolor blue - 3]]]
;    [ask n-of 1 patches with [pycor > 50] [ask n-of 1 patches in-radius 1 [set pcolor blue - 3]]
;end
...
;;;;;;;; SETUP FUNCTION: create people
to set-people
  ifelse dense?
  [create-people 50 [setxy random-xcor 80 set shape "person" set color yellow set size 2.5 set health 100 set hunger random 100 set infected? false]]
  [create-people 10 [setxy random-xcor 80 set shape "person" set color yellow set size 2.5 set health 100 set hunger random 100 set infected? false]]
end



;;;;;;;; GO FUNCTION: people move
to move-people
  ask people [
    if [pcolor] of patch-ahead 0.1 = green + 1
    [set heading (180 - heading)]
    if abs [pxcor] of patch-ahead 0.1 = 0
    [set heading ( - heading)]
    fd 0.05 right random 0.5 left random 0.5

  set hunger hunger - 0.01
...
to set-new-people
  ask patch 50 75 [
    if ticks mod 500 = 4 [
      sprout-people random 4 [set shape "person" set color yellow set size 2.5 set health 100 set hunger random 100 set infected? false]
    ]
  ]
end


;;;;; if people step on an infected patch, there is a small chance they become infected
;to infected-people
;    ask people [
;      set color 13 set health health - 0.02
;      if ticks mod 100 = 2 [set infected? true]
;    ]
;  ]
;end

;;;; if people come into contact with an infected rat, if they are hungry, they will eat it and become infected
to people-eat-rats
...
;;;;;;;; SETUP FUNCTION: create rats
to set-rats
  create-rats 20 [setxy random-xcor 10 set shape "mouse side" set color grey set size 1.5 set infected? false set hunger 100 set age 100]
end



;;;;;;;; GO FUNCTION: rats move
to move-rats
  ask rats [
    ifelse hunger < 70

    [if abs [pycor] of patch-ahead 0.1 = 0
    [set heading (180 - heading)]
    if abs [pxcor] of patch-ahead 0.1 = 0
    [set heading ( - heading)]
    fd 0.05 right random 5 left random 5
    ]

    [if abs [pycor] of patch-ahead 0.1 = 0 or [pcolor] of patch-ahead 0.1 = 8.9
    [set heading (180 - heading)]
    if abs [pxcor] of patch-ahead 0.1 = 0
    [set heading ( - heading)]
    fd 0.05 right random 0.5 left random 0.5
...
;;;;; rats reproduce most in warm weather
to set-new-rats
  ifelse warm-weather?
  [ask patch 50 15 [
    if ticks mod 400 = 4 [
      sprout-rats random 4 [set shape "mouse side" set color grey set size 1.5 set infected? false set hunger 100 set age 100]
      sprout-rats 1 [set shape "mouse side" set color black set size 1.5 set infected? true set hunger 100 set age 100]
    ]]
  ]
  [ask patch 50 15 [
    if ticks mod 600 = 4 [
...
;;;;; if rats land in farmland, they are able to become less hungry
to rats-eat
  ask rats [
    if pcolor = 53 [
      set pcolor 36
...
;;;;; have an infected rat change a red or blue patch
to infect-village
  ask rats [
    if infected? [
      if pcolor = pink - 1 or pcolor = blue + 1 [
        if ticks mod 3000 = 2 [set pcolor 14]
      ]
      if pcolor = white - 1 [
        if ticks mod 10000 = 2 [set pcolor 14 set infected true]
      ]
    ]
  ]
end
