; Demonstrates delay -- people stop for some amount of time when they collide with a house
; v2 works the same as v1 but better commented

breed [people person]               ; define breeds (plural and singular required)
breed [houses house]
breed [products product]
globals [second-counter]            ; resets to 0 every 5 ticks
people-own [making-sale waiting-time]
turtles-own [age]  ; not used in program -- just shows you how to make a variable owned by all turtles


; make your code very modular, and follow a consistent format
; comments are very helpful!

to setup
  clear-all                    ; clears agents, variables, etc
  reset-ticks                  ; starts clock over at 0 ticks
  make-houses                    ; calls the 'make-houses' routine below
  make-people                    ; calls the 'make-people' routine below
end

to go
  tick
  set second-counter ticks mod 5  ;  resets second-couter to 0 every 5 ticks (not used in this program)
  people-walk
  houses-sit
end

; create people and houses
...
to make-people
  create-people number-of-people [
    ...
    set shape "person"
;    set color yellow
    set waiting-time -1         ; always initialize your variables.  -1 says waiting-time starts out not in use.
    set making-sale false       ; people start out not making a sale
  ]
end
...
; make people stop at houses when making a sale

to people-walk                           ; this routine makes people stop between 5 and 20 ticks if they land on a house
  ask people
    [if not making-sale              ; if I'm not already paused at a house, see if I just landed on one
      [if any? houses-here               ; if there is a house on this patch, set waiting-time and making-sale
                                         ;   houses-here is the agent-set of all houses on this patch
                                         ;   any? returns true if agent-set contains at least one house, false otherwise
        [ set waiting-time 5 + random 15
          set making-sale true
          let my-house one-of houses-here
          hatch-products 1
 ;         hatch-products 1 [set color [color] of my-house]
        ]
      ]
      ifelse waiting-time >= 0            ; if I am waiting at a house, count down the timer
        [set waiting-time waiting-time - 1
          if waiting-time = -1              ; if at end of wait, making-sale is now false
              [ set making-sale false
                forward 2                   ; move away so don't find this house again
              ]
        ]

        [ left  random 30                 ; if not waiting, move forward with a little randomness
          right random 30
          forward 2
        ]
...
to houses-sit ; does nothing at the moment!
