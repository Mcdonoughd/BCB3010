; v2 demonstrates computing a second-counter from ticks if each second is 5 ticks
; flowers change size briefly at beginning of every second
breed [people person]               ; define breeds (plural and singular required)
breed [houses house]
globals [second-counter]
...
to setup
  ...
  make-houses                    ; calls the 'make-flowers' routine below
  make-people                    ; calls the 'make-bees' routine below
end

to go                          ; making separate 'bees-fly' and 'flowers-make-nectar' routines makes it
  tick
  set second-counter ticks mod 5
  people-walk                     ;    very clear that that's all this simulation is doing
  houses-sit
end
...
; make bees and flowers behave and interact
...
to people-walk                           ; this routine makes people stop between 5 and 20 ticks if they land on a house
  ask people
    [if not making-sale
      ...
      ifelse waiting-time >= 0            ; if waiting, count down the timer
      [set waiting-time waiting-time - 1
        if waiting-time = -1   [ set making-sale false forward 2]   ; if at end of wait, making-sale is now false
                                                                    ; move away so don't find this house again
      ]
      [ left  random 30
        right random 30
        forward 2
