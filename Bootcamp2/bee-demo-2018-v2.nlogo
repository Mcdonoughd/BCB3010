...
breed [bees bee]               ; define breeds (plural and singular required)
...
to setup
  ...
  make-flowers                    ; calls the 'make-flowers' routine below
  make-bees                    ; calls the 'make-bees' routine below
end

to go                          ; making separate 'bees-fly' and 'flowers-make-nectar' routines makes it
  tick
  set second-counter ticks mod 5
  bees-fly                     ;    very clear that that's all this simulation is doing
  flowers-make-nectar
end
...
to bees-fly                           ; this routine makes the bees fly around and bump into flowers
  ask bees [
    if any? flowers-here              ; if there are flowers on this patch, bee grows a little
    [ set size size + 0.1
      ask one-of flowers-here         ; then choose one flower at random for color change
      [ ifelse color = cyan
        [set color orange]
        [set color cyan]
      ]
    ]
    set size size - 0.01              ; bee shrinks at every tick due to energy loss
    if size < 0.1 [die]               ; if get too small, dies
    left  random 30                   ; then move basically straight but with some randomness
    right random 30
    forward 2
  ]
end

to flowers-make-nectar
  ask flowers [
    ifelse second-counter = 0
      [set size 4]
      [set size 3]
  ]
end
